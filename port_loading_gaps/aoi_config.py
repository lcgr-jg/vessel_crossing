...
TERMINAL_TEMPLATES: dict[str, dict[str, Any]] = {
    ...
    "berth_slots_jetty": {
        ...
        "tunable_detect_keys": [
            ...
            "slot_diff_frac / slot_diff_blob_frac — weak-vessel combo path",
        ],
    },
    # Multi-SPM terminal (CPC): open-water slots — water-only scoring + cross-SPM filter
    "berth_slots_spm": {
        "mode": "berth_slots",
        "sensor": "s2",
        "detect": {
    ...
    # One SPM per polygon (Das, Zirku, Um Lulu): tight single slot, no cross-SPM filter
    "single_spm_polygon": {
        "mode": "berth_slots",
        "sensor": "s2",
        "detect": {
        ...
        "tunable_detect_keys": [
            "slot_hull_frac — lower if dark-green hulls missed",
            "slot_min_blob_frac — raise to reduce FPs; lower to catch pale hulls",
            "slot_water_only — keep True for SPMs",
            "use_ndwi_water / ndwi_water_min — NDWI water mask inside slot",
        ],
    },
...
DEFAULT_KPLER_HINTS: dict[str, str] = {
    ...
    "cpc": 'LocationSpec("???", kind="zone")  # confirm CPC zone name in Kpler',
}
